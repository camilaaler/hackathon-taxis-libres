...
//										->requireSelect2()
		...
		foreach ($this->_getIconLibraryPreparator()->getIconFontCSSList() as $slug => $path) {
			$iconStyles[ $slug ] = $this->_getWplayer()->getFrameworkUrl() . $path;
		}

		$iconStyles = $this->_getWPLayer()->apply_filters( 'ff_font_uris', $iconStyles);

		foreach ($iconStyles as $slug => $path) {
			$this->_getStyleEnqueuer()->addStyle('ff-font-'.$slug, $path);
		}
	}
...
<!--
 ...
 -->
				...
				<div class="ff-modal-library-item-details-settings-row description-font">
					<div class="ff-modal-library-item-details-settings-th">Font Family</div>
					<div class="ff-modal-library-item-details-settings-td"><p></p></div>
				</div>
