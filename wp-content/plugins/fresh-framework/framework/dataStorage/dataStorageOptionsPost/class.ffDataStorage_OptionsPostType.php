...
################################################################################
	...
	private function _initNamespace( $namespace ) {
		...
		if( !isset( $this->_loadedNamespaces[ $namespace ] ) ) {
			$optionsPost = $this->_loadedPosts[ $namespace ] = $this->_getPostLayer()->getPostGetter()->addArg('post_status', 'any')->getPostBySlug( $namespace, ffDataStorage_OptionsPostTypeRegistrator::OPTIONS_POST_TYPE_SLUG  );
