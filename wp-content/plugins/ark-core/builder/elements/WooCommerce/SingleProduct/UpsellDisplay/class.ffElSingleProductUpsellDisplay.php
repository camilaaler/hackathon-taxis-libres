...
class ffElSingleProductUpsellDisplay extends ffAbstractElProductLoop {
	protected $_wcElementTitle;
	protected $_wcElementQuery;
	protected $_wcElementContent;

	protected function _initData() {
		$this->_setData( ffThemeBuilderElement::DATA_ID, 'woocommerce-single-product-upsell-display');
		$this->_setData( ffThemeBuilderElement::DATA_NAME, 'WC - Single - Upsell');
		...
		$this->_setData( ffThemeBuilderElement::DATA_PICKER_TAGS, 'woocommerce, wc, upsell display, single product');
	...
	protected function _getElementGeneralOptions( $s ) {
		...
		$s->addOption(ffOneOption::TYPE_TEXT, 'title', '', __( 'You may also like&hellip;', 'woocommerce' ) );
		$s->addElement(ffOneElement::TYPE_TABLE_DATA_END);
...
//					setup_postdata( $GLOBALS['post'] =& $post_object );
	...
	protected function _render( ffOptionsQueryDynamic $query, $content, $data, $uniqueId ) {
		...
		if( $is_wrapped ) {
			echo '<'.$tag.' class="ark-woocommerce-single-product-upsell-display">';
		}
		...
		woocommerce_upsell_display( $limit, 999, $orderby, $order );
