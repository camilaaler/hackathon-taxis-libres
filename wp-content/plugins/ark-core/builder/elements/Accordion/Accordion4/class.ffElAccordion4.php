...
class ffElAccordion4 extends ffElAccordion2 {

	protected function _initData() {
		$this->_setData( ffThemeBuilderElement::DATA_ID, 'accordion-4');
		$this->_setData( ffThemeBuilderElement::DATA_NAME, esc_attr( __('Accordion 4', 'ark' ) ) );
	...
	protected function _getBGColorSelectors(){
		return array(
			...
			'.panel-title > a:before' => 'header-bg-color',
			'.panel-title > a:hover' => 'header-bg-color-hover',
			'.panel-title > a:hover:before' => 'header-bg-color-hover',
	...
	protected function _render( ffOptionsQueryDynamic $query, $content, $data, $uniqueId ) {
		...
		$this->_renderBasicElement($query, $content, $data, $uniqueId, 4);
