...
class ffElAlert2 extends ffThemeBuilderElementBasic {
	protected function _initData() {
		$this->_setData( ffThemeBuilderElement::DATA_ID, 'alert-2');
		$this->_setData( ffThemeBuilderElement::DATA_NAME, esc_attr( __('Alert 2', 'ark' ) ) );
	...
	protected function _getElementGeneralOptions( $s ) {
		...
		$s->addElement( ffOneElement::TYPE_TABLE_START );
			...
			$s->addElement( ffOneElement::TYPE_TABLE_DATA_START, '', ark_wp_kses( __('Icon', 'ark' ) ) );
				...
				;
				$s->addOptionNL( ffOneOption::TYPE_COLOR_PICKER_WITH_LIB, 'icon-bg-color', '', '#e55973')
				...
				echo '<div class="alert-box-body">';
					...
					foreach( $query->get('content') as $oneItem ) {
						switch ($oneItem->getVariationType()) {
							...
							case 'description':
								$this->_renderCSSRule( 'color', $oneItem->getColor('color'), '.alert-box-paragraph', true );
