...
class WCRA_Wpml
{
	...
	public function get_current_language_id($item_id, $post_type = "product", $return_original = true)
	{
		if(!class_exists('SitePress'))
			$item_id;
		...
		global $sitepress;
		if(function_exists('icl_object_id'))
			$item_translated_id = icl_object_id($item_id, $post_type, $return_original);
		else
			$item_translated_id = apply_filters( 'wpml_object_id', $item_id, $post_type, $return_original );
	...
	function register_single_string($id, $text, $prefix = "email_subject_" )
	{
		do_action( 'wpml_register_single_string', 'woocommerce-role-by-amount-spent', 'wcra_'.$prefix.$id, $text);
	} 
}
?>